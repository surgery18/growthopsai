<template>
	<div v-if="selectedProjectId === null" class="view-section flex flex-col items-center justify-center h-[60vh] text-center">
		...
		<p class="text-gray-400 mb-6 max-w-md">
			To use the dashboard and other project tools, you need to select an active project from the dropdown in the sidebar, or
			create and complete an intake for a new project.
		</p>
	...
	<div v-else id="view-dashboard" class="view-section grid grid-cols-12 gap-6 pb-20">
		<div class="col-span-12 mb-6 space-y-4">
			...
			>
				<template v-if="isGenerating">
					<div class="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin mr-3" />
			...
			<div v-if="dailyStrategy" class="glass-panel p-6 rounded-xl border border-brand-primary/20 bg-brand-primary/5">
				...
				<div class="prose prose-invert max-w-none text-gray-300 text-sm max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
		...
		<div class="col-span-12">
			<div class="flex items-center justify-between mb-4">
				...
				<RouterLink to="/settings" class="text-xs text-brand-accent hover:text-white flex items-center">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				<div
					...
					:class="[agent.borderColor, agent.status === 'idle' ? 'opacity-75' : '']"
				>
					...
					<div class="flex items-center gap-3 mb-2">
						<div
							...
							:class="[agent.bgPulse, agent.status === 'active' ? 'animate-pulse' : '']"
						></div>
						<h3 class="font-bold" :class="agent.status === 'idle' ? 'text-gray-400' : 'text-gray-200'">
							{{ agent.name }}
						</h3>
					</div>
					<p class="text-xs h-8 line-clamp-2" :class="agent.status === 'idle' ? 'text-gray-500' : 'text-gray-400'">
						{{ agent.activity }}
					</p>
					<div v-if="agent.status !== 'idle'" class="mt-2 w-full bg-gray-800 rounded-full h-1">
						<div
							class="h-1 rounded-full"
							:class="[agent.progressColor, agent.animateBar ? 'animate-pulse' : '']"
			...
			<div class="glass-panel rounded-xl flex flex-col h-full">
				<div class="p-5 border-b border-white/10 flex justify-between items-center">
					<div class="flex items-center gap-2">
						...
						<span class="bg-brand-primary/20 text-brand-accent text-xs px-2 py-0.5 rounded border border-brand-primary/30">
							Standard Lane
						</span>
					</div>
					<div class="flex gap-2 items-center">
						...
						<RouterLink to="/approvals" class="text-xs text-brand-accent hover:text-white">
				...
				<div v-if="approvalItem" class="p-6">
					<div class="mb-4 flex items-center justify-between">
						<div class="flex items-center gap-2">
							<span class="text-xs font-mono text-gray-500 uppercase">Context:</span>
							<span class="text-xs bg-gray-800 text-gray-300 px-2 py-1 rounded">Campaign: Product Launch</span>
							<span class="text-xs bg-gray-800 text-gray-300 px-2 py-1 rounded">Platform: {{ approvalItem.platform }}</span>
						</div>
						<div v-if="cancelMode" class="flex items-center gap-2 animate-in fade-in slide-in-from-right-4">
							<span class="text-xs text-red-400 font-bold uppercase">End Campaign?</span>
							<button
					...
					<div class="bg-white/5 border border-white/10 rounded-lg p-6 mb-6">
						<div class="flex gap-4">
							...
							<div class="flex-1">
								...
								<div v-if="approvalPreview.previewPosts.length" class="space-y-4">
									<div v-if="approvalPreview.strategyText" class="text-sm text-gray-400 bg-black/50 p-2 rounded mb-4">
								...
								<div v-else class="text-gray-300 text-base leading-relaxed mb-4">
					...
					<div v-if="rejectMode" class="bg-white/5 p-4 rounded-lg border border-white/10 animate-in fade-in zoom-in-95">
						<div class="space-y-4">
							<div>
								<label class="text-xs text-gray-400 font-medium ml-1 block mb-1">Reason for Revision</label>
								<div class="relative">
									<select
							...
							<div class="flex items-center justify-end gap-2">
								<Button size="sm" variant="ghost" @click="rejectMode = false">Cancel</Button>
								<Button size="sm" variant="danger" :disabled="isSubmitting" @click="handleReject(approvalItem.id)">
		...
		<div class="col-span-12 lg:col-span-4 space-y-6">
			<div v-if="interestedEvents.length > 0" class="glass-panel p-5 rounded-xl">
					...
					>
						<div class="flex justify-between items-start gap-2">
							<div class="flex-1 min-w-0">
								<h4 class="font-medium text-white text-sm truncate">{{ event.name }}</h4>
								<p class="text-xs text-gray-500 truncate">
									{{ event.event_date || "TBD" }} • {{ event.location || "Location TBD" }}
								</p>
							</div>
							<a
			...
			<div class="bg-[#0D0E12] border border-white/10 rounded-xl p-4 font-mono text-xs h-64 overflow-hidden flex flex-col">
				...
				<div class="overflow-y-auto space-y-2 text-gray-400 flex-1 scroll-hide">
					<div v-for="log in auditLog" :key="log.id" class="flex gap-2 leading-tight">
						<span class="text-gray-600 flex-shrink-0">
							[{{ formatLogTime(log.timestamp) }}]
						</span>
						<span class="flex-1">
							<span :class="actorColor(log.actor)">{{ formatActor(log.actor) }}</span>
							<span :class="actionColor(log.action)"> {{ log.action?.toLowerCase() }}</span>
							<span class="text-gray-500"> {{ log.entity_type }} #{{ log.entity_id }}</span>
							<span v-if="logDetail(log)" class="text-gray-400 ml-1">— {{ logDetail(log) }}</span>
						</span>
					</div>
					<div class="flex gap-2">
						<span class="text-gray-600">[{{ formatLogTime(Date.now()) }}]</span>
						<span class="text-white typing-effect">Listening for events...</span>
					</div>
				</div>
			</div>
		</div>
...
const getDailyCooldownKey = (projectId: number | string) => `daily-material-requested-${projectId}`
...
const handleRequestMaterial = async () => {
	...
	try {
		...
		if (res.ok) {
			...
			toast.success("New materials generated & added to queue!", { id: "daily-material-success" })
			fetchDashboard()
		} else {
...
const logDetail = (log: AuditLog) => {
	...
	try {
		...
		if (details.content) {
			return details.content.substring(0, 50) + (details.content.length > 50 ? "..." : "")
		}
		return ""
	} catch {
