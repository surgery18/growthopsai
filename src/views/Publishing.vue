<template>
	<div class="space-y-8 max-w-5xl mx-auto pb-20">
		<div class="flex items-center justify-between">
			...
			<RouterLink to="/approvals" class="text-sm text-brand-accent hover:text-white">View Approvals</RouterLink>
		</div>

		<div class="space-y-4">
			<h2 class="text-xl font-semibold text-white">Ready to Publish</h2>
			<Card v-if="readyPosts.length === 0" class="p-6 text-gray-400">No approved posts ready to publish.</Card>
			<Card v-for="post in readyPosts" :key="post.id" class="p-6 space-y-4">
				<div class="flex items-center justify-between">
					<div>
						<div class="text-sm text-gray-400">Campaign: {{ campaignName(post.campaign_id) }}</div>
						<div class="text-xs text-gray-500">Post #{{ post.id }}</div>
					</div>
					<Button :disabled="publishingId === post.id" @click="handlePublish(post)">
						<Send class="w-4 h-4 mr-2" /> {{ publishingId === post.id ? "Publishing..." : "Publish" }}
					</Button>
				</div>
				<XPostPreview
				...
				/>
				<div v-if="previewFor(post).strategyText" class="text-sm text-gray-400 bg-black/50 p-2 rounded">
		...
		<div class="space-y-4">
			...
			<Card v-if="publishedPosts.length === 0" class="p-6 text-gray-400">No published posts yet.</Card>
			<Card v-for="post in publishedPosts" :key="post.id" class="p-6 space-y-4">
				<div class="flex items-center justify-between">
					<div>
						<div class="text-sm text-gray-400">Campaign: {{ campaignName(post.campaign_id) }}</div>
						<div class="text-xs text-gray-500">Published Post #{{ post.id }}</div>
					</div>
				</div>
				<XPostPreview
					v-for="(preview, idx) in previewFor(post).previewPosts"
...
const fetchDashboard = async () => {
	...
	const data = (await response.json()) as { posts: Post[]; campaigns: any[]; publishJobs: any[] }
	posts.value = data.posts
	campaigns.value = data.campaigns
	publishJobs.value = data.publishJobs
}

const campaignName = (campaignId: number) => campaigns.value.find((c) => c.id === campaignId)?.name || ""
...
const handlePublish = async (post: Post) => {
	...
	try {
		const res = await fetch(`/api/publish-jobs/${job.id}/run`, { method: "POST" })
		if (res.ok) {
			fetchDashboard()
		} else {
