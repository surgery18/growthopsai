<template>
	<div>
		<div
			...
			<div class="bg-[#12141A] border border-white/10 rounded-2xl max-w-sm w-full p-6 shadow-2xl text-center">
		...
		<div
			...
			<div class="bg-[#12141A] border border-white/10 rounded-2xl max-w-md w-full p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
				<div class="flex items-center gap-3 mb-4">
					...
					<div>
						<h3 class="text-lg font-bold text-white">Finalize Project Profile?</h3>
						<p class="text-sm text-gray-400">This action cannot be undone.</p>
					</div>
				</div>

				<p class="text-gray-300 text-sm mb-4">
					By submitting, this version will be <strong class="text-white">locked</strong> and sent to our AI
					agents for analysis. You can create a new version later if you need to make changes.
				</p>
		...
		<div v-if="!draft && !isLoading" class="view-section flex flex-col items-center justify-center h-[50vh] text-center">
		...
		<div v-else-if="draft" class="view-section max-w-6xl mx-auto pb-40">
			<div class="flex items-center justify-between mb-8 sticky top-0 bg-[#0B0C0F]/95 backdrop-blur z-20 py-4 border-b border-white/5">
				<div class="flex items-center gap-4">
					...
					<div>
						<h1 class="text-xl font-bold text-white flex items-center gap-2">
							{{ project?.name }}
							<span class="text-xs font-normal text-gray-500 bg-white/5 px-2 py-0.5 rounded border border-white/5">
				...
				<div class="flex items-center gap-4">
					<span class="text-xs text-gray-500 font-mono">
						<span v-if="isSaving" class="flex items-center gap-1 text-yellow-500">
							<Loader2 class="w-3 h-3 animate-spin" /> Saving...
						</span>
						<span v-else-if="lastSaved" class="flex items-center gap-1">
							<CheckCircle2 class="w-3 h-3 text-green-500" /> Saved {{ lastSaved.toLocaleTimeString() }}
						</span>
			...
			<div class="grid grid-cols-12 gap-8">
				<div class="col-span-12 lg:col-span-3">
					<div class="bg-[#12141A] border border-white/10 rounded-xl p-4 sticky top-24">
						<div class="mb-6 px-2">
				...
				<div class="col-span-12 lg:col-span-9">
					<div class="bg-[#12141A] border border-white/10 rounded-xl p-6 md:p-8 min-h-[600px] flex flex-col relative overflow-hidden">
						<div class="mb-8 pb-6 border-b border-white/5">
							<div class="flex items-center gap-3 mb-2">
								<span class="bg-brand-primary/20 text-brand-primary text-xs font-bold px-2 py-1 rounded">
									Step {{ currentStep + 1 }} of {{ STEPS.length }}
								</span>
								<h2 class="text-2xl font-bold text-white">{{ STEPS[currentStep].title }}</h2>
							</div>
						...
						<div class="flex-1">
								...
								<div class="grid grid-cols-2 gap-6">
									<div class="col-span-2">
										<label class="block text-sm font-medium text-gray-300 mb-1">
											Project Name *
											<span v-if="errors['project_basics.project_name']" class="text-red-500 text-xs ml-2">
										...
										<input
											...
											@input="updateField('project_basics', 'project_name', $event.target.value)"
									...
									<div class="col-span-2 md:col-span-1">
										<label class="block text-sm font-medium text-gray-300 mb-1">
											Industry *
											<span v-if="errors['project_basics.industry']" class="text-red-500 text-xs ml-2">
												{{ errors['project_basics.industry'] }}
											</span>
										</label>
										<select
											:value="formData.project_basics?.industry || 'Technology'"
											@change="updateField('project_basics', 'industry', $event.target.value)"
									...
									<div class="col-span-2 md:col-span-1">
										<label class="block text-sm font-medium text-gray-300 mb-1">Website URL</label>
										<input
											type="url"
											:value="formData.project_basics?.website_url || ''"
											@input="updateField('project_basics', 'website_url', $event.target.value)"
											placeholder="https://example.com"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
									<div class="col-span-2 md:col-span-1">
										<label class="block text-sm font-medium text-gray-300 mb-1">Primary Contact Name</label>
										<input
											type="text"
											:value="formData.project_basics?.primary_contact_name || ''"
											@input="updateField('project_basics', 'primary_contact_name', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
									<div class="col-span-2 md:col-span-1">
										...
										<input
											type="email"
											:value="formData.project_basics?.primary_contact_email || ''"
											@input="updateField('project_basics', 'primary_contact_email', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
								</div>
							</div>
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Short Description *
										<span v-if="errors['business_summary.short_description']" class="text-red-500 text-xs ml-2">
											{{ errors['business_summary.short_description'] }}
										</span>
									</label>
									<p class="text-xs text-gray-500 mb-2">1-3 sentences describing what your company does.</p>
									<textarea
										:value="formData.business_summary?.short_description || ''"
										@input="updateField('business_summary', 'short_description', $event.target.value)"
										rows="3"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										...
										<span
											v-if="errors['business_summary.product_or_service_description']"
											class="text-red-500 text-xs ml-2">
											{{ errors['business_summary.product_or_service_description'] }}
										</span>
									</label>
									<textarea
										:value="formData.business_summary?.product_or_service_description || ''"
										@input="updateField('business_summary', 'product_or_service_description', $event.target.value)"
										rows="4"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div class="grid grid-cols-2 gap-6">
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">
											Primary Goal *
											<span v-if="errors['business_summary.primary_goal']" class="text-red-500 text-xs ml-2">
												{{ errors['business_summary.primary_goal'] }}
											</span>
										</label>
										<select
											:value="formData.business_summary?.primary_goal || 'Leads'"
											@change="updateField('business_summary', 'primary_goal', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none">
									...
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Geographic Focus</label>
										<input
											type="text"
											placeholder="e.g. United States, Global, NYC"
											:value="formData.business_summary?.geographic_focus || ''"
											@input="updateField('business_summary', 'geographic_focus', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
								</div>
							</div>
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										...
										<span v-if="errors['target_audience.ideal_customer_description']" class="text-red-500 text-xs ml-2">
											{{ errors['target_audience.ideal_customer_description'] }}
										</span>
									</label>
									<p class="text-xs text-gray-500 mb-2">Who is your absolute best customer?</p>
									<textarea
										:value="formData.target_audience?.ideal_customer_description || ''"
										@input="updateField('target_audience', 'ideal_customer_description', $event.target.value)"
										rows="4"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Top Pain Points *
										<span v-if="errors['target_audience.top_pain_points']" class="text-red-500 text-xs ml-2">
									...
									<textarea
										...
										@input="updateField('target_audience', 'top_pain_points', $event.target.value)"
										rows="4"
								...
								<div class="grid grid-cols-2 gap-6">
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Demographics</label>
										<input
											type="text"
											placeholder="Age 25-45, Male, High Income"
											:value="formData.target_audience?.demographics || ''"
											@input="updateField('target_audience', 'demographics', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Exclusions</label>
										<input
											type="text"
											placeholder="We do NOT target..."
											:value="formData.target_audience?.exclusions || ''"
											@input="updateField('target_audience', 'exclusions', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
								</div>
							</div>
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Primary Offer *
										<span v-if="errors['offer_funnel.primary_offer']" class="text-red-500 text-xs ml-2">
											{{ errors['offer_funnel.primary_offer'] }}
										</span>
									</label>
									<p class="text-xs text-gray-500 mb-2">What specifically are you selling right now?</p>
									<textarea
										:value="formData.offer_funnel?.primary_offer || ''"
										@input="updateField('offer_funnel', 'primary_offer', $event.target.value)"
										rows="2"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div class="grid grid-cols-2 gap-6">
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">
											...
											<span v-if="errors['offer_funnel.primary_call_to_action']" class="text-red-500 text-xs ml-2">
										...
										<input
											...
											:value="formData.offer_funnel?.primary_call_to_action || ''"
											@input="updateField('offer_funnel', 'primary_call_to_action', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Pricing Range</label>
										<input
											type="text"
											placeholder="$100 - $500"
											:value="formData.offer_funnel?.pricing_range || ''"
											@input="updateField('offer_funnel', 'pricing_range', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
								</div>
							</div>
							...
							<div
								...
								<div class="space-y-6">
									<label class="block text-sm font-medium text-gray-300 mb-1">Voice Personality</label>
									<div v-for="slider in VOICE_SLIDERS" :key="slider.key" class="space-y-2">
										...
										<input
											...
											@input="updateField('brand_voice', slider.key, parseInt($event.target.value, 10))"
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Core Values *
										<span v-if="errors['brand_voice.brand_values']" class="text-red-500 text-xs ml-2">
									...
									<input
										...
										@input="updateField('brand_voice', 'brand_values', $event.target.value)"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div class="grid grid-cols-2 gap-6">
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Words We Like</label>
										<input
											type="text"
											placeholder="Modern, Slick, Fast"
											:value="formData.brand_voice?.words_we_like || ''"
											@input="updateField('brand_voice', 'words_we_like', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Words to AVOID</label>
										<input
											type="text"
											placeholder="Cheap, Discount, Hack"
											:value="formData.brand_voice?.words_we_avoid || ''"
											@input="updateField('brand_voice', 'words_we_avoid', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
								</div>
							</div>
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Logo URL *
										<span v-if="errors['visual_brand.logo_url']" class="text-red-500 text-xs ml-2">
									...
									<div class="flex gap-2">
										<input
											...
											@input="updateField('visual_brand', 'logo_url', $event.target.value)"
								...
								<div class="grid grid-cols-2 gap-6">
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Brand Colors</label>
										<input
											type="text"
											placeholder="#FF0000, #0000FF"
											:value="formData.visual_brand?.brand_colors || ''"
											@input="updateField('visual_brand', 'brand_colors', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
									<div>
										<label class="block text-sm font-medium text-gray-300 mb-1">Fonts</label>
										<input
											type="text"
											placeholder="Inter, Roboto"
											:value="formData.visual_brand?.fonts || ''"
											@input="updateField('visual_brand', 'fonts', $event.target.value)"
											class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
									</div>
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">Image Style</label>
									<select
										:value="formData.visual_brand?.image_style || 'Photographic'"
										@change="updateField('visual_brand', 'image_style', $event.target.value)"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none">
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Competitors (Min 2) *
										<span v-if="errors['competitors.competitor_list']" class="text-red-500 text-xs ml-2">
											{{ errors['competitors.competitor_list'] }}
										</span>
									</label>
									<textarea
										:value="formData.competitors?.competitor_list || ''"
										@input="updateField('competitors', 'competitor_list', $event.target.value)"
										rows="3"
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										Key Differentiators *
										<span v-if="errors['competitors.differentiators']" class="text-red-500 text-xs ml-2">
											{{ errors['competitors.differentiators'] }}
										</span>
									</label>
									<textarea
										:value="formData.competitors?.differentiators || ''"
										@input="updateField('competitors', 'differentiators', $event.target.value)"
										rows="3"
										placeholder="Why do customers choose you over them?"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">We Win Because...</label>
									<input
										type="text"
										:value="formData.competitors?.we_win_because || ''"
										@input="updateField('competitors', 'we_win_because', $event.target.value)"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
							</div>
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-3">
										Active Channels *
										<span v-if="errors['channels.active_channels']" class="text-red-500 text-xs ml-2">
									...
									<div class="grid grid-cols-2 gap-3">
										<label
											...
											<input
												...
												:checked="formData.channels?.active_channels?.includes(channel) || false"
												@change="toggleChannel(channel, $event.target.checked)" />
											<span class="text-white text-sm">{{ channel }}</span>
										</label>
									</div>
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">Content Topics to Focus On</label>
									<textarea
										:value="formData.channels?.topics_focus || ''"
										@input="updateField('channels', 'topics_focus', $event.target.value)"
										rows="2"
							...
							<div
								...
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">
										...
										<span v-if="errors['examples.liked_examples']" class="text-red-500 text-xs ml-2">
											{{ errors['examples.liked_examples'] }}
										</span>
									</label>
									<p class="text-xs text-gray-500 mb-2">Links to posts or profiles that inspire you.</p>
									<textarea
										:value="formData.examples?.liked_examples || ''"
										@input="updateField('examples', 'liked_examples', $event.target.value)"
										rows="4"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-300 mb-1">Do NOT Post...</label>
									<p class="text-xs text-gray-500 mb-2">Banned topics, claims, or styles.</p>
									<textarea
										:value="formData.examples?.banned_content || ''"
										@input="updateField('examples', 'banned_content', $event.target.value)"
										rows="2"
										class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:ring-1 focus:ring-brand-primary outline-none" />
								</div>
							</div>
							...
							<div v-if="currentStep === 9" class="space-y-6">
								<div class="bg-brand-primary/10 border border-brand-primary/20 rounded-xl p-6">
									<h3 class="text-brand-primary font-bold mb-2">Review Your Information</h3>
									<p class="text-gray-300 text-sm">
										Review the data below before submitting. Click "Submit Intake" when you're ready to
						...
						<div class="mt-12 pt-6 border-t border-white/10 flex justify-between">
							<Button variant="secondary" :disabled="currentStep === 0" @click="prevStep">
								<ChevronLeft class="w-4 h-4 mr-2" />
								Previous
							</Button>
							<Button v-if="currentStep === STEPS.length - 1" class="bg-green-600 hover:bg-green-500 text-white" @click="handleSubmit">
...
const isStepComplete = (data: any, step: number): boolean => {
	switch (step) {
		case 0:
			return hasField(data, "project_basics", "project_name") && hasField(data, "project_basics", "industry")
		case 1:
			return (
				hasField(data, "business_summary", "short_description") &&
				hasField(data, "business_summary", "product_or_service_description") &&
				hasField(data, "business_summary", "primary_goal")
			)
		case 2:
			return hasField(data, "target_audience", "ideal_customer_description") && hasField(data, "target_audience", "top_pain_points")
		case 3:
			return hasField(data, "offer_funnel", "primary_offer") && hasField(data, "offer_funnel", "primary_call_to_action")
		case 4:
			return hasField(data, "brand_voice", "brand_values")
		case 5:
			return hasField(data, "visual_brand", "logo_url")
		case 6:
			return hasField(data, "competitors", "competitor_list") && hasField(data, "competitors", "differentiators")
		case 7:
			return data?.channels?.active_channels && data.channels.active_channels.length > 0
		case 8:
...
const saveDraft = async () => {
	...
	isSaving.value = true
	try {
		await fetch(`/api/projects/${project.value.id}/intake/${draft.value.id}`, {
			method: "PUT",
			headers: {
			...
			body: JSON.stringify({ data_json: formData.value, step: currentStep.value }),
		})
...
const validateStep = (step: number) => {
	...
	const requireField = (section: string, field: string, label: string) => {
		if (!formData.value[section]?.[field] || formData.value[section][field].toString().trim() === "") {
	...
	switch (step) {
		...
		case 1:
			...
			requireField("business_summary", "product_or_service_description", "Product/Service Description")
			requireField("business_summary", "primary_goal", "Primary Goal")
			break
		case 2:
			requireField("target_audience", "ideal_customer_description", "Ideal Customer Profile")
			requireField("target_audience", "top_pain_points", "Top Pain Points")
			break
		case 3:
			requireField("offer_funnel", "primary_offer", "Primary Offer")
			requireField("offer_funnel", "primary_call_to_action", "Primary Call to Action")
			break
		case 4:
		...
		case 6:
			...
			break
		case 7:
			if (!formData.value.channels?.active_channels || formData.value.channels.active_channels.length === 0) {
				newErrors["channels.active_channels"] = "Please select at least one channel"
				isValid = false
			}
			break
		case 8:
...
const handleSubmit = () => {
	...
	requireField("project_basics", "industry", "Industry")
	requireField("business_summary", "short_description", "Short Description")
	requireField("business_summary", "product_or_service_description", "Product/Service Details")
	requireField("business_summary", "primary_goal", "Primary Goal")
	requireField("target_audience", "ideal_customer_description", "Ideal Customer Profile")
	requireField("target_audience", "top_pain_points", "Top Pain Points")
	requireField("offer_funnel", "primary_offer", "Primary Offer")
...
const confirmSubmit = async () => {
	...
	await saveDraft()
	try {
		const res = await fetch(`/api/projects/${project.value.id}/intake/${draft.value.id}/submit`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				"x-org-id": "demo-org",
...
const showUploadNotice = () => {
	window.alert("File upload simulation: Please host your logo and paste the link for now.")
}
...
const toggleChannel = (channel: string, checked: boolean) => {
	...
	const next = checked ? [...current, channel] : current.filter((c: string) => c !== channel)
