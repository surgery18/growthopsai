<template>
	<div class="view-section max-w-7xl mx-auto pb-20">
		...
		<div v-if="projects.length === 0" class="text-center py-20 bg-white/5 rounded-xl border border-white/10 border-dashed">
			<Folder class="w-12 h-12 text-gray-600 mx-auto mb-4" />
			<h3 class="text-lg font-medium text-white mb-2">No projects yet</h3>
			<p class="text-gray-400 mb-6">Create your first project to get started with GrowthOps AI.</p>
			<Button variant="secondary" class="bg-gray-700 hover:bg-gray-600" @click="isModalOpen = true">
				Create Project
			</Button>
		</div>
		<div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			<Card v-for="project in projects" :key="project.id" class="hover:border-brand-primary/50 transition-colors group">
				<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
					<CardTitle class="text-lg font-bold text-white truncate">{{ project.name }}</CardTitle>
					<Badge v-if="project.status === 'ACTIVE'" variant="success">Active</Badge>
					<Badge v-else variant="default">Draft</Badge>
				</CardHeader>
				<CardContent>
					<div class="text-sm text-gray-400 mb-4 h-5">{{ project.industry }}</div>
		...
		<div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
			<div class="bg-[#12141A] border border-white/10 rounded-xl w-full max-w-md p-6 shadow-2xl relative">
				...
				<form @submit.prevent="handleCreate">
					...
					<div class="flex justify-end gap-3 mt-8">
						<button type="button" class="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors" @click="isModalOpen = false">
							Cancel
						</button>
...
const handleCreate = async () => {
	...
	try {
		const res = await fetch("/api/projects", {
			...
			body: JSON.stringify({ name: newProjectName.value, industry: industry.value }),
		})
		...
		router.push(`/projects/${data.project.id}/intake`)
	} catch (error) {
		console.error(error)
		alert("Network error")
	} finally {
