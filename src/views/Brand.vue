	...
	<div class="space-y-8 pb-20">
		<div
			...
			<p class="text-gray-400 mb-6 max-w-md">
				Select an active project to view its brand bible, or complete an intake
				to generate brand data.
			</p>
				...
				<Card class="p-6 space-y-4">
					<div>
						...
						<p class="text-gray-200 mt-1">
							{{ brandVoiceSummary || "No brand voice data yet." }}
						</p>
					</div>
					<div>
						<label class="text-xs text-gray-500 uppercase font-bold"
						...
						>
						<p class="text-gray-200 mt-1">
							{{ targetPersona || "No target persona captured yet." }}
						</p>
					</div>
					<div>
						<label class="text-xs text-gray-500 uppercase font-bold"
						...
						>
						<p class="text-gray-200 mt-1">
							{{ valueProp || "No value proposition defined yet." }}
						</p>
					</div>
				</Card>
			</div>
				...
				<Card class="p-6 space-y-6">
					<div>
						<div class="flex items-center gap-2 mb-2">
							<div
								class="h-4 w-4 rounded-full bg-green-500 flex items-center justify-center text-black font-bold text-[10px]">
								✓
							</div>
						...
						<div class="flex flex-wrap gap-2">
							...
							<span v-if="doSay.length === 0" class="text-xs text-gray-500"
								>No approved phrases yet.</span
							>
						</div>
					</div>
					<div>
						<div class="flex items-center gap-2 mb-2">
							<div
								class="h-4 w-4 rounded-full bg-red-500 flex items-center justify-center text-white font-bold text-[10px]">
								✕
							</div>
						...
						<div class="flex flex-wrap gap-2">
							...
							<span v-if="doNotSay.length === 0" class="text-xs text-gray-500"
								>No restricted phrases yet.</span
							>
						</div>
					</div>
				</Card>
...
<script setup lang="ts">
	...
	const buildProductDescriptionFromIntake = (
		...
		const details = toStringOrNull(
			businessSummary.product_or_service_description,
		)
	...
	const fetchBrand = async () => {
		...
		try {
			...
			if (data.source === "intake") {
				const intakeData = data.data_json
					? parseJson<Record<string, any>>(data.data_json)
					: null
	...
	const brandVoiceSummary = computed(() => {
		if (brandVoice.value?.core_values?.length)
			return brandVoice.value.core_values.join(", ")
		if (brandVoice.value?.words_to_use?.length)
			return brandVoice.value.words_to_use.join(", ")
		if (brandVoice.value?.personality)
			return formatPersonality(brandVoice.value.personality)
		return null
	})
