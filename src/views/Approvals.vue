	...
	<div class="space-y-12 max-w-4xl mx-auto pb-20">
		...
		<div class="flex items-center gap-2 mb-4">
			<Badge variant="default" class="text-sm"
				>{{ pendingPosts.length }} Reviews Pending</Badge
			>
		</div>
		...
		<div class="space-y-4">
			...
			<Card
				...
				class="p-12 flex flex-col items-center justify-center text-gray-500 border-dashed">
				...
				<div class="border-l-4 border-l-brand-primary h-full">
						...
						<div class="flex items-center gap-2">
							<div
								...
								<Button
									...
									>Confirm</Button
								>
								<Button size="sm" variant="ghost" @click="cancelId = null"
									>Back</Button
								>
							</div>
							<Button
					...
					<CardContent class="space-y-4">
						<div v-if="previewFor(item).previewPosts.length" class="space-y-4">
							<div
								v-if="previewFor(item).strategyText"
						...
						<div
							class="flex items-center justify-end gap-3 pt-2 border-t border-white/5">
							<div
								...
								<div class="space-y-2">
									<label class="text-xs text-gray-400 font-medium ml-1"
								...
								<div class="flex items-center justify-end gap-2 pt-2">
									<Button size="sm" variant="ghost" @click="rejectId = null"
										>Cancel</Button
									>
									<Button
										size="sm"
			...
			<Card
				...
				<CardHeader>
					<div class="flex items-center gap-3">
						<Badge variant="warning" class="border-yellow-500 text-yellow-500"
							>REVISION IN PROGRESS</Badge
						>
				...
				<CardContent class="space-y-4">
					<div
						class="bg-yellow-500/10 border border-yellow-500/20 p-3 rounded text-yellow-200 text-sm">
						<span class="font-bold">Rejection Reason:</span>
						{{ revisionReason(item.id) }}
					</div>
...
<script setup lang="ts">
	...
	const pendingPosts = computed(() =>
		posts.value.filter((post) => post.status === "INTERNAL_APPROVED"),
	)
	const readyPosts = computed(() =>
		posts.value.filter((post) => post.status === "CLIENT_APPROVED"),
	)
	const revisionPosts = computed(() =>
		posts.value.filter((post) => post.status === "CLIENT_CHANGES_REQUESTED"),
	)
	...
	const handleApprove = async (id: number) => {
		try {
			...
			toast.error("Network error approving post")
		}
	}
	...
	const handleReject = async (id: number) => {
		...
		try {
			...
			toast.error("Network error rejecting post")
		}
	}
	...
	const revisionReason = (postId: number) => {
		return (
			revisions.value.find((r: any) => r.post_id === postId)?.reason ||
			"Reason not recorded"
		)
	}
