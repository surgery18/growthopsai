<template>
	<div
		class="flex h-screen flex-col bg-[#0a0a0f] text-slate-200 font-sans overflow-hidden">
		<header
			class="flex items-center justify-between border-b border-white/5 bg-black/20 px-6 py-4 backdrop-blur-md shrink-0">
			<div class="flex items-center gap-3">
				<div
					class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-brand-primary to-purple-600 shadow-lg shadow-brand-primary/20">
		...
		<div class="flex-1 flex gap-6 p-6 overflow-hidden">
			<div class="w-64 flex flex-col gap-2 shrink-0">
				<div class="flex items-center justify-between mb-2">
					...
					<Button size="sm" variant="ghost" @click="handleCreateCampaign"
						>+</Button
					>
				</div>
				...
				<button
					...
					<div class="flex justify-between items-center">
						...
						<div
							v-if="isCampaignLocked(campaign.status)"
				...
				<div
					...
					class="text-sm text-gray-500 italic p-4 text-center border border-dashed border-gray-800 rounded">
					No campaigns yet. <br />Click + to start.
				</div>
			</div>

			<Card
				class="flex-1 flex flex-col p-0 overflow-hidden bg-black/20 border-white/10">
				<div
					:key="selectedDoId"
					ref="scrollRef"
					...
					@scroll="handleScroll">
					<div
						v-if="parsedMessages.length === 0 && !isThinking"
					...
					<div
						...
						:class="
							entry.msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''
						">
						<Avatar
							:fallback="entry.msg.role === 'user' ? 'U' : 'AI'"
							:class="
								entry.msg.role === 'user' ? 'bg-brand-primary' : 'bg-purple-600'
							" />
						<div
							...
							">
							<div
								v-if="entry.parsed.kind === 'assignments'"
								class="space-y-3 w-full">
								<p class="text-sm font-medium text-brand-primary">
									Internal Comms:
								</p>
								<div
									class="bg-black/20 border border-white/5 rounded-lg overflow-hidden text-[10px] text-gray-400">
									<div class="p-2 space-y-1">
										<div
											v-for="(job, idx) in entry.parsed.assignments"
											:key="idx"
											class="flex gap-2">
								...
								<MarkdownContent
									v-if="entry.parsed.reply"
									:content="entry.parsed.reply" />
							</div>
							...
							<div
								v-else-if="entry.parsed.kind === 'integrated'"
								class="space-y-3 w-full animate-in zoom-in-95 duration-500">
								<div
									class="bg-black/40 border border-green-500/30 rounded-lg overflow-hidden">
									<div
										class="px-3 py-2 border-b border-green-500/10 bg-green-500/5 flex items-center gap-2">
										<CheckCircle class="w-3 h-3 text-green-500" />
										<span
											class="text-xs font-bold text-green-500 uppercase tracking-wider"
								...
								<div class="text-xs text-gray-500 italic mt-2 text-center">
									Campaign Approved. Thread Locked.
								</div>
							</div>
							...
							<div
								v-else-if="entry.parsed.kind === 'json'"
								class="text-xs overflow-auto text-gray-500 bg-black/20 p-2 rounded">
					...
					<div v-if="streamingContent" class="flex gap-4 max-w-3xl">
						...
						<div
							class="p-4 rounded-2xl bg-white/5 text-gray-200 rounded-tl-none border border-white/5">
							<MarkdownContent
								:content="extractReply(streamingContent) || '...'" />
						</div>
					</div>

					<div
						v-if="isThinking && !streamingContent"
						class="flex gap-4 max-w-3xl animate-pulse">
						<Avatar fallback="AI" class="bg-purple-600 opacity-50" />
						<div
							class="p-4 rounded-2xl bg-white/5 text-gray-400 rounded-tl-none border border-white/5 text-xs italic flex items-center gap-2">
							<span
								class="w-2 h-2 bg-purple-500 rounded-full animate-bounce"></span>
							<span
								class="w-2 h-2 bg-purple-500 rounded-full animate-bounce [animation-delay:0.2s]"></span>
							<span
								class="w-2 h-2 bg-purple-500 rounded-full animate-bounce [animation-delay:0.4s]"></span>
							Elena is analyzing...
						</div>
					</div>
				</div>
				...
				<div class="p-4 bg-white/5 border-t border-white/5 shrink-0">
					<div
						...
						<p class="text-xs opacity-70">
							{{ lockedMessage }}<br />Please create a new campaign for a new
							request.
						</p>
					</div>
					<form v-else class="flex gap-4" @submit.prevent="handleSend">
						...
						<Button
							type="submit"
							:disabled="!input.trim() || !selectedDoId"
							variant="primary">
							<Send class="w-4 h-4" />
						</Button>
					</form>
...
<script setup lang="ts">
	...
	const allMessages = computed(() => {
		...
		if (optimisticUserMsg.value) {
			const exists = filtered.some(
				(m) => m.timestamp === optimisticUserMsg.value?.timestamp,
			)
	...
	const selectedCampaign = computed(() =>
		campaigns.value.find((c) => c.do_id === selectedDoId.value),
	)
	...
	const extractReply = (text: string) => {
		...
		const matchIntegrated = text.match(
			/"integrated_output":\s*"([^"\\]*(?:\\.[^"\\]*)*)"?/,
		)
		if (matchIntegrated) {
			try {
		...
		if (
			...
			!text.includes('"reply"') &&
			!text.includes('"integrated_output"')
		) {
			return ""
		}
		return text
	...
	const isCampaignLocked = (status: string) =>
		[
			...
			"CANCELLED",
			"CLIENT_VISIBLE",
		].includes(status?.toUpperCase?.() || status)
	...
	const submitNewCampaign = async () => {
		...
		try {
			...
			if (!res.ok) {
				...
				throw new Error(
					errorData.message || errorData.error || "Campaign create failed",
				)
			}
