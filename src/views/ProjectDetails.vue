	...
	<div>
		<div
			...
			<div
				...
				<p class="text-gray-300 text-sm mb-6">
					This will permanently delete this draft version. Your other versions
					(active, submitted) will not be affected.
				</p>
		...
		<div class="view-section max-w-6xl mx-auto pb-20">
			<div class="flex items-center gap-4 mb-8">
				...
				<div>
					<RouterLink
						v-if="draftIntake && project"
						:to="`/projects/${project.id}/intake`">
						<Button
							class="bg-yellow-500/10 text-yellow-500 border-yellow-500/20 hover:bg-yellow-500/20">
				...
				<div class="col-span-12 lg:col-span-8 space-y-8">
					<Card
						...
						<div class="p-6 relative">
							<h3
								class="text-lg font-bold text-brand-primary mb-4 flex items-center gap-2">
							...
							<div
								v-if="activeIntake.ai_summary"
								class="prose prose-invert prose-sm max-w-none">
								<MarkdownContent :content="activeIntake.ai_summary" />
							</div>
							<p v-else class="text-gray-400 italic">
								Analysis pending or not generated.
							</p>
							<div
								class="mt-6 pt-4 border-t border-brand-primary/20 text-xs text-brand-primary/70 flex items-center gap-2">
								<CheckCircle2 class="w-3 h-3" />
								Based on Intake v{{ activeIntake.version_num }} • Activated
								{{ formatDate(activeIntake.activated_at) }}
							</div>
						</div>
					</Card>
					<div
						v-else
						class="border border-dashed border-white/10 rounded-xl p-12 text-center bg-white/5">
						<FileText class="w-12 h-12 text-gray-600 mx-auto mb-4" />
						<h3 class="text-lg font-medium text-white mb-2">
							No Active Profile
						</h3>
						<p class="text-gray-400 mb-6">
							Complete the intake wizard to activate this project/project
							profile.
						</p>
					...
					<div v-if="activeIntake" class="space-y-8">
						<div class="space-y-4">
							...
							<div
								class="bg-[#12141A] border border-white/10 rounded-xl p-6 overflow-hidden">
								<pre
									class="text-xs text-gray-500 overflow-auto max-h-96 font-mono"
									v-text="intakeDataJson"></pre>
							</div>
						</div>
						...
						<div v-if="activeIntake.ai_context_pack_json" class="space-y-4">
							...
							<div
								class="bg-[#12141A] border border-brand-primary/20 rounded-xl p-6 overflow-hidden relative">
								<div
									class="absolute top-0 right-0 p-2 bg-brand-primary/10 rounded-bl-xl text-xs text-brand-primary font-mono">
									Generated by Gemini
								</div>
								<pre
									class="text-xs text-brand-primary/80 overflow-auto max-h-96 font-mono"
									v-text="aiContextJson"></pre>
							</div>
						</div>
					</div>
				</div>
	...
	const handleCreateDraft = async () => {
		...
		try {
			const res = await fetch(
				`/api/projects/${project.value.id}/intake/draft`,
				{
					method: "POST",
			...
			if (res.ok) {
				...
				if (data.draftId) {
					router.push(`/dashboard/projects/${project.value.id}/intake`)
				}
			} else {
				const data = (await res.json()) as { message?: string; error?: string }
				toast.error(data.message || data.error || "Failed to create draft")
			}
		} catch (error) {
			toast.error("Network error creating draft")
		}
	}
	...
	const confirmDeleteDraft = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				const data = (await res.json()) as { message?: string; error?: string }
				toast.error(data.message || data.error || "Failed to delete draft")
			}
		} catch (error) {
		...
		try {
			...
			if (res.ok) {
				const indexRes = await fetch(
					`/api/projects/${project.value.id}/knowledge/sync`,
					{
						method: "POST",
						headers: {
				...
				if (indexRes.ok) {
					...
					toast.warning("Reprocessed but knowledge sync failed")
					await fetchDetails()
				}
			} else {
				const data = (await res.json()) as { message?: string; error?: string }
				toast.error(data.message || data.error || "Reprocessing failed")
			}
		} catch (error) {
			toast.error("Network error reprocessing")
		} finally {
